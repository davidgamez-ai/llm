...
(0, node_test_1.test)("encode() is case-sensitive", () => {
    strict_1.default.deepEqual(tokenizer.encode("hello"), [UNKNOWN_TOKEN_ID]);
});
(0, node_test_1.test)("encode() substitutes the <|unk|> id for a token that was never seen during training", () => {
    strict_1.default.deepEqual(tokenizer.encode("banana"), [UNKNOWN_TOKEN_ID]);
});
(0, node_test_1.test)("encode() substitutes the <|unk|> id in place, alongside known tokens", () => {
    strict_1.default.deepEqual(tokenizer.encode("Hello banana world"), [
        5,
        UNKNOWN_TOKEN_ID,
        11,
    ]);
...
(0, node_test_1.test)("train() appends the special tokens after the regular vocabulary", () => {
    // The fixture has 12 unique regular tokens (ids 0-11), so the special
    // tokens should continue the sequence at 12 and 13.
    strict_1.default.deepEqual(tokenizer.encode("<|unk|> <|endoftext|>"), [12, 13]);
});
(0, node_test_1.test)("train() replaces the previous vocabulary rather than merging it", () => {
    ...
    try {
        ...
        strict_1.default.deepEqual(tokenizer.encode("Hello"), [3]);
