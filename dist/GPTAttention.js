...
class GPTAttention {
    ...
    calculate(embeddings) {
         ...
         */
        const contextVectors = attentionWeights.map((weights) => Array.from({ length: this.weightMatrixColumns }, (_, column) => weights.reduce((sum, weight, token) => sum + weight * values[token][column], 0)));
