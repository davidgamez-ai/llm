...
function writeFixture(dir: string, contents: string): string {
...
beforeEach((t) => {
  ...
  (t as TestContext).mock.method(console, "log", () => {});
...
test("train() replaces the previous vocabulary rather than merging it", () => {
  const secondFixtureDir = fs.mkdtempSync(
    path.join(os.tmpdir(), "tokenizer-test-")
  );
