...
class Context {
   ...
   */
  getContext(embeddings: number[][], attentionScores: number[][]): number[][] {
    // Step 1: note the shape of the input so the calculation can be
    // followed from the start.
    console.log(
      `Context| Calculating context vectors for ${embeddings.length} tokens, each embedding ${embeddings[0]?.length ?? 0} values wide.`
    );
